...
class Post {
  ...
  factory Post.fromXml(content) {
    ...
    var municipality = content
        .findAllElements("item")
        .map((node) => node.findElements("kommun").single.text);
    var county = content
        .findAllElements("item")
        .map((node) => node.findElements("lan").single.text);
    var sourceInfo = content
        .findAllElements("item")
        .map((node) => node.findElements("sourceInfo").single.text);
    var sourceUrl = content
        .findAllElements("item")
        .map((node) => node.findElements("url").single.text);
    // Average, min, max data
    var averageTemp = content
        .findAllElements("item")
        .map((node) => node.findElements("average").single.text);
    var minTemp = content
        .findAllElements("item")
        .map((node) => node.findElements("min").single.text);
    var maxTemp = content
        .findAllElements("item")
        .map((node) => node.findElements("max").single.text);
    ...
    return Post(
      ...
      amm: "min " +
          ...
          "° ◦ medel " +
          averageTemp.single.toString() +
          "° ◦ max " +
          maxTemp.single.toString() +
          "°",
